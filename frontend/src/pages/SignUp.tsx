...
export default function SignUp() {
	...
	const openNotificationWithIcon = (type: NotificationType) => {
		...
		switch (type) {
			case 'success':
				messageTitle = '成功';
				messageDescription = 'ユーザー登録が成功しました。';
				break;
			case 'error':
				messageTitle = '失敗';
				messageDescription = 'ユーザー登録が失敗しました。';
				break;
			default:
				messageTitle = '失敗';
				messageDescription = 'ユーザー登録が失敗しました。';
	...
	const handleSignUp = async () => {
		...
		await endpoint.signup(email, password).then((response) => {
			...
			<Row justify={'center'} align={'middle'} className='h-screen bg-SignBg bg-cover bg-no-repeat bg-left'>
				<Col className='max-w-[650px] w-11/12'>
					...
					<Row justify={'end'} align={'middle'} gutter={10} className='mt-[5px]'>
						...
						<Col>
							<Link underline href='/' className='!text-main text-[12px]'>
								ここからログイン
							</Link>
						</Col>
					</Row>
