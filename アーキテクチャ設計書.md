...
- **Backend (NestJS)**：
    ...
    - `members`, `organization`, `orders`, `products`, `commissions`などのモジュールは、基本的に共通データアクセスロジックやビジネスロジックを提供し、フロントエンドが管理者・会員いずれであっても同一APIで対応可能（認可ロジックでデータ範囲を��限）。
...
- **UI拡張**：
    ...
    会員向け機能を拡張する場合、既存`members`��`orders`モジュールにロール別認可ロジックを加える。
